...
public class Game {
	...
	public static void play() {
		...
		while (Board.getForest().size() > 0) {
			...
			try {
				...
				int option =
						Integer.parseInt(keyboard.readLine("Enter the number for the action:"));
				boolean succesfullMove = false;
				switch (option) {
					case 1:
						int position = Integer.parseInt(keyboard
								.readLine("Enter the number of the card you want to take:"));
					...
					case 2:
						...
						break;
					case 3:
						String cookMush = keyboard.readLine(
								"What ingredients are you cooking? Type the position of the cards in your hand (use commas to separate):");
						...
						for (int k = 0; k < splittedStringOfInts.length; k++) {
							...
							cookingmushrooms
									.add(currentPlayer.getHand().getElementAt(inputInt - 1));
						}
						...
						break;
					case 4:
						String mushType = keyboard.readLine(
								"What type of mushrooms are you willing to sell? Type mushroom name:");
						int mushNumber = Integer.parseInt(
								keyboard.readLine("How many are willing to sell? Type number:"));
				...
				System.out.println(
						"This is not the appropriate format. Avoid white spaces and characters when a number is expected.");
			}
		}
	...
	private static void displayGameStatus() {
		System.out.println("\nThere are " + Board.getForestCardsPile().pileSize()
				+ " cards in the Forest pile");
